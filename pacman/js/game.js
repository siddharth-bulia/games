...
function ready() {
	LOCK = true;
	GAME_START_LOCK = true;
	if (window.webkit && !GAMEOVER && SCORE === 0) {
		window.webkit.messageHandlers.gameStateEvent.postMessage(JSON.stringify({ state: "start", score: 0 }));	// send score to iOS client
	}
	message("Swipe to start>>");

	playReadySound();
	if (!GAME_START_LOCK) setTimeout("go()", "2100");		// tweak around this value to change start game interval
}
function go() {
	playSirenSound();
...
function pauseGame(hide_message) {

	if (!PAUSE) {
		stopAllSound();
		PAUSE = true;

		if (hide_message !== false) message("pause");
...
function gameover() {
	GAMEOVER = true;
	if (window.webkit) {
		window.webkit.messageHandlers.gameStateEvent.postMessage(JSON.stringify({ state: "end", score: SCORE }));	// send score to iOS client
	}

	if(typeof jsInterfaceName !== 'undefined'){
   jsInterfaceName.updateScoreToWebView(SCORE);
  }
	message("game over");
	stopTimes();
...
function score(s, type) {
	...
	if (window.webkit && !GAMEOVER) {
		window.webkit.messageHandlers.scoreEvent.postMessage(JSON.stringify({ score: SCORE }));	// send score to iOS client
	}
